...
- メモは外部のテキストファイル(.txt/.md/.csv等)としても読み書きできる。文字コード(UTF-8 / BOM付きUTF-8 / Shift_JIS / UTF-16LE)と改行コード(CRLF / LF / CR)は読み込み時に判別し、保存時も同じ形へ戻す
- メモの選択範囲に、クリップボードの整形ルール(登録済みの置換ルール)をそのまま適用できる。選択範囲を直前に使っていたアプリへ直接入力したり、マクロの入力ステップとしてメモを参照させることもできる。メモは本文全体をまとめてコピーしたり、装飾・画像ごと複製したりできる
- メモにはスクリーンショット等の画像を貼り付けられる(本文とは別ファイルとして保持)。本文をMarkdownとして横に表示することもできる
