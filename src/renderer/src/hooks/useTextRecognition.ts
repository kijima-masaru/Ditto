...
/**
 * 録画枠の範囲を静止画1枚としてキャプチャし、OCRで文字起こしするためmainへ渡す。
 * 撮影処理自体はuseScreenshotと同じdesktopCapturer+<video>+<canvas>の組み合わせを使う。
 * OCR自体と結果の確認・編集は別ウィンドウ(textRecognitionWindow.ts)で行うため、
 * data URLはこのフックの状態には残さずopenTextRecognitionEditorへそのまま渡す。
 */
export function useTextRecognition(): TextRecognitionApi {
  ...
  const capture = useCallback(async () => {
    ...
    try {
      ...
      await window.api.openTextRecognitionEditor(canvas.toDataURL('image/png'))
    } catch (e) {
