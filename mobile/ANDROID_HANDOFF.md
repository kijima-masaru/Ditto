...
1. **`expo-camera`のQRスキャン**。エミュレータの仮想カメラにQRコードを映すのが現実的でないため未検証。**物理実機での確認が必要な唯一の項目**。コード入力モードで代替できるため、ブロッカーではない。
...
1. Androidアプリを起動→ヘッダー右の**接続アイコン(鎖)**を押して接続モーダルを開く。「QRコード読み取り」でPC画面のQRを読むか、「コード入力」でPCのIPアドレスと6桁コードを入力する。
2. PC側に確認ダイアログ(「"○○" がこのPCとの連携を要求しています」)が出るので「許可」を押す。
3. 接続できるとモーダルが閉じ、接続アイコンが緑になる。
4. ヘッダー右の**歯車アイコン**を押して設定モードを開き、ボタン数を選び、各ボタンに定型文・マクロを割り当てる。**候補はPCに登録されている全件**(ピン留めによる絞り込みは廃止した)。
5. PC側の任意のテキスト入力欄(メモ帳等)にフォーカスした状態で、定型文ボタンをタップ→実際にテキストが入力されることを確認する。
6. マクロボタンを長押し(0.5秒)→PC側でマクロが実行され、`Notification`(「スマホからマクロ「○○」が実行されました」)が表示されることを確認する。
7. PC設定画面の「スマホ連携」→ペアリング済み一覧に端末が表示されることを確認し、「×」で失効させる。Android側で次の操作(再接続やトリガー)が失敗することを確認する。

### 4. `adb reverse`で確認する場合(同一Wi-Fiが使えない環境向け・エミュレータもこれ)

PC側の`remoteServer.ts`は変更不要。`adb reverse`でスマホ側の`localhost:58211`をPCの`58211`番ポートへトンネルするだけで、同一LAN接続と全く同じWebSocketサーバーに到達できる。**エミュレータでも実機と同様に機能する。**

1. AndroidをUSBデバッグでPCに接続(エミュレータなら起動)した状態で、PC側ターミナルで以下を実行:
   ```bash
   adb reverse tcp:58211 tcp:58211
   ```
2. 接続モーダルの「コード入力」でIPアドレスに **`127.0.0.1`** 、コード欄にPC側の6桁コードを入力して「接続する」。
3. 以降の確認手順(定型文/マクロトリガー・デバイス失効)は同一Wi-Fi接続時と同じ。

> 以前は「USB接続」タブ(host=`127.0.0.1`固定)を用意していたが、UI刷新の際に廃止した。IP欄に`127.0.0.1`と入力すれば同じことができるため。iOSシミュレータからMacのlocalhostへ繋ぐ場合も同様。
...
## UI刷新(2026-08-18)で変わったこと

モックに合わせて画面構成を作り直した。**通信・暗号処理には一切手を入れていない。**

- **常にホーム画面(ボタングリッド)を表示する**。以前は未ペアリング時に`PairingScreen`を全画面表示していたが、接続は**ヘッダーの接続アイコンから開くモーダル**に移した。`PairingScreen.tsx`は削除し、`components/ConnectModal.tsx`に置き換えている。
- **接続手段はQRとコード入力の2つ**。「USB接続」タブは廃止(IP欄に`127.0.0.1`と入れれば同じ)。
- **ボタンは設定モードで自由に構成する**。個数は1〜12、割り当てはPCから受け取った定型文・マクロの全件から選ぶ。設定は`lib/buttonConfig.ts`が端末に永続化する(SecureStoreに相乗り。AsyncStorageを足すとネイティブ再ビルドが必要になるため)。
- **PC側`buildItemsMessage()`の`pinned`フィルタを外した**。割り当て候補を全件にするため。この変更により、PC側でピン留めを敷き直す運用は不要になった。
- **配色をPC本体のダークテーマに揃えた**(`src/theme.ts`に`App.css`の`:root[data-theme='dark']`から移植)。以前の紫基調は廃止。
- アイコンは`components/Icons.tsx`で`View`を組んで自作している。絵文字では`color`が効かず接続状態を色で表せず、アイコンライブラリを足すとフォント読み込みのネイティブ依存が増えるため。

### レイアウトで踏んだReact Nativeの罠(2件)

1. **`onLayout`が返すのはpaddingを含んだ外側のサイズ**。padding付きのViewで直接測ると、その分ボタンが大きく算出されて3列目が入りきらず2列に折り返した。余白の内側に測定用のViewを挟んで解決している。
2. **`borderStyle`はスタイル配列から出し入れしても再描画されないことがある**。未割り当て(破線)→割り当て済み(実線)にしても破線のまま残った。`borderStyle: slot ? 'solid' : 'dashed'`と**どちらの状態でも必ず値を指定する**ことで解決。
...
- **QRスキャンが反応しない**: `mobile/src/components/ConnectModal.tsx`の`CameraView`設定を見直す。Expo SDKのバージョンによる既知の不具合の可能性があるため、手入力モードで代替できるか確認する。
...
- Android側画面: `mobile/src/screens/HomeScreen.tsx`, `mobile/src/components/ConnectModal.tsx`, `mobile/src/components/SettingsModal.tsx`
