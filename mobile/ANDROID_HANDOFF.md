...
| `listItems`(定型文・マクロの取得。全件+pinnedフラグ) | ✅ |
...
### itemsの絞り込みとpinnedフラグ

`items`は**ピン留めで絞らず全件届く**(PC側`buildItemsMessage()`)。各項目には`pinned`フラグが付いており、スマホ側は用途で使い分けている:

- **自動配置**(まだボタンを設定していないとき / `gridLayout.ts`の`autoItems()`)は`pinned`のものだけを並べる。カスタマイズしていないユーザーの見え方を変えないため。
- **設定モードの割り当て候補**は全件を出す。候補をピン留め済みに絞ると、結局PC側でピン留めを敷き直す手間が残ってしまうため。

`autoItems()`の判定は`=== true`ではなく`pinned !== false`にしてある。`pinned`を送らない古いPC(ピン留め済みだけを返していた頃のDitto)に繋いだ場合、値がundefinedになって**全件が弾かれ自動配置が空になる**ため、「falseと明示されたものだけ除く」形にしている。
...
1. AndroidをUSBデバッグでPCに接続した状態で、PC側ターミナルで以下を実行:
   ...
   ```
2. アプリの接続モーダル(ヘッダーの接続アイコンから開く)の「コード入力」で、IPアドレスに`127.0.0.1`、ポートに`58211`を入力する。
   > 以前あった「USB接続」タブ(host/port固定)はUI刷新にあたり削除した。`adb reverse`でトンネルを張る手順自体は変わらないので、接続先を手で入力すれば同じ経路で繋がる。
