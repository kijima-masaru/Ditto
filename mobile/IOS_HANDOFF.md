...
**2026-08-18: iOSシミュレータ(iPhone 17 Pro / iOS 26.5)での検証が完了しました。** ビルド・起動からペアリング、リモート操作、デバイス失効まで一通り動作しています。**Android版の検証で見つかった暗号処理の修正はiOSでもそのまま有効**で、iOS固有の暗号バグは見つかりませんでした。Android側の検証後に入った自動再接続・ペアリング拒否時の復帰(`ANDROID_HANDOFF.md`のバグ5・6)も、iOSで同様に動作することを確認済みです。
...
| 「手入力」モードでのペアリング(LAN IP指定) | ✅ `ws://192.168.0.39:58211/ws`で成立 |
| **ペアリング拒否時にスピナーから復帰しエラーを表示**(ANDROID_HANDOFFのバグ6の修正) | ✅ 誤った6桁コードで確認 |
| **PC側を落としたときの未接続バナー+ボタン無効化**(同バグ5の修正) | ✅ |
| **PC側再起動後の自動再接続(無操作での復帰)**(同上) | ✅ サーバーをkill→再起動し、アプリに触れずに復帰して定型文トリガーが通ることを確認 |
...
3. **実機を同一Wi-Fiに置いた接続**。`127.0.0.1`だけでなくLAN IP(`ws://192.168.0.39:58211/ws`)でのペアリングも確認済みだが、**iOSシミュレータはホストMacのネットワークスタックをそのまま使うため、これは「実機がWi-Fi経由で別のPCに繋ぐ」ことの検証にはならない**。2と同時に確認すること。
...
- デバイス失効の直後、アプリはホーム画面のまま「未接続」表示で項目も残る。失効を検知するのは次回の認証時(Android版と同じ)。
...
- **`wsClient.ts`を直したのに挙動が変わらない**: `ANDROID_HANDOFF.md`に記載のあるMetroのFast Refreshの罠がiOSでも同じく当てはまる。`App.tsx`が`useRef`で保持している`RemoteClient`インスタンスはFast Refreshでは作り直されないため、画面の修正だけが反映されてクライアント側のロジックは古いまま動く。`adb shell am force-stop`にあたる操作はこちら:
  ```bash
  xcrun simctl terminate <UDID> com.ditto.remote && xcrun simctl launch <UDID> com.ditto.remote
  ```
- **古いMetroが残っていて混乱する**: 検証を繰り返していると`expo run:ios`が「Skipping dev server」と言って既存のMetroに相乗りする。どのMetroが生きているか分からなくなったら`lsof -ti:8081 | xargs kill -9`で一度全部落としてから`npx expo start`し直すこと。
- **接続できない**: モックサーバーが起動しているか(`lsof -i:58211`)。実機の場合はローカルネットワーク権限を許可したか(iOSの設定→アプリ名→ローカルネットワーク)。
...
2. Windows機が使える状態になったら、本物のPC側Dittoに対してiOSアプリを繋ぎ、モックサーバーでは確認できない相互運用を最終確認する。
